...
} from '/var/task/UnifiedAI/flofaction-nextjs-site/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '/var/task/UnifiedAI/flofaction-nextjs-site/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/UnifiedAI/flofaction-nextjs-site')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/UnifiedAI/flofaction-nextjs-site',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/UnifiedAI/flofaction-nextjs-site/.netlify/dist/run/handlers/server.js')
